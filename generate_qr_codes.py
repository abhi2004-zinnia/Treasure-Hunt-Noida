"""
Generate labelled QR codes for Treasure Hunt Noida task pages.

Owner: Abhishek — https://github.com/abhi2004-zinnia/Treasure-Hunt-Noida

Default base URL targets GitHub Pages for this repository. Override for your host:

    set TREASURE_HUNT_BASE_URL=https://your-domain.example/path
...
# GitHub Pages URL for this repo (no trailing slash)
_DEFAULT_PAGES_BASE = "https://abhi2004-zinnia.github.io/Treasure-Hunt-Noida"
...
def main():
    raw = os.environ.get("TREASURE_HUNT_BASE_URL", _DEFAULT_PAGES_BASE).strip().rstrip("/")
    base_url = raw or _DEFAULT_PAGES_BASE

    print("Generating QR codes - Treasure Hunt Noida (Abhishek)")
    print("=" * 50)
    print(f"Base URL for task links: {base_url}")
    print("  (set TREASURE_HUNT_BASE_URL to override)")
    print()

    os.makedirs("qr_codes", exist_ok=True)

    tasks = [
        {"url": f"{base_url}/task1.html", "label": "Task 1 - Start Your Hunt!", "filename": "qr_codes/task1_qr.png"},
        {"url": f"{base_url}/task2.html", "label": "Task 2 - Second Clue", "filename": "qr_codes/task2_qr.png"},
        {"url": f"{base_url}/task3.html", "label": "Task 3 - Third Clue", "filename": "qr_codes/task3_qr.png"},
        {"url": f"{base_url}/task4.html", "label": "Task 4 - Fourth Challenge", "filename": "qr_codes/task4_qr.png"},
        {"url": f"{base_url}/task5.html", "label": "Task 5 - WINNER!", "filename": "qr_codes/task5_qr.png"},
    ]

    print("Creating QR codes for tasks:")
    print("-" * 40)
    ...
    print("GAME FLOW:")
    print("-" * 30)
    print("- Task 1: Teams enter their Team ID -> get a unique code for the next step")
    print("- Tasks 2-5: Teams enter Team ID + the code from the previous task -> next clue")
    print("- Codes are per team (stored in Firestore); each team's sheet is different")
    print()
    print("FIREBASE:")
    print("-" * 30)
    print("- Real-time multi-device tracking")
    print("- Live admin dashboard updates")
    print("- Registration via register.html (first five teams)")
    print()
